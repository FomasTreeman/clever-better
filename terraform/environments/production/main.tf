...
module "ecr" {
  ...
  environment            = var.environment
  image_tag_mutability   = "IMMUTABLE" # Production uses immutable tags
  max_image_count        = 20          # Keep more images in production
...
module "ecs_tasks" {
  ...
  # Resource allocation (production - higher)
  bot_cpu    = var.bot_cpu
...
module "ecs_services" {
  ...
  # Service scaling (production - high availability)
  bot_desired_count = 2
  bot_min_capacity  = 2
  bot_max_capacity  = 5
  ml_desired_count  = 2
  ...
  # Production options - disable exec for security
